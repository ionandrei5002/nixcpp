        ...
        switch (this._errorState.state) {
            case 'init': {
                ...
                if (result) {
                    ...
                    const filepath = path.isAbsolute(filename) ? filename : util.lightNormalizePath(path.join(this.sourceDir, filename));
...
class CompileOutputConsumer {
    constructor() {
        ...
        this._ghs_re = /^\"(.*)\",\s+(?:(?:line\s+(\d+)\s+\(col\.\s+(\d+)\))|(?:At end of source)):\s+(?:fatal )?(remark|warning|error)\s+(.*)/;
        this._gcc_re = /^(.*):(\d+):(\d+):\s+(?:fatal )?(\w*)(?:\sfatale)?\s?:\s+(.*)/;
        this._gnu_ld_re = /^(.*):(\d+)\s?:\s+(.*[^\]])$/;
        this._msvc_re = /^\s*(?!\d+>)?\s*([^\s>].*)\((\d+|\d+,\d+|\d+,\d+,\d+,\d+)\):\s+((?:fatal )?error|warning|info)\s+(\w{1,2}\d+)\s*:\s*(.*)$/;
        this._ghsDiagnostics = [];
        this._gccDiagnostics = [];
        this._gnuLDDiagnostics = [];
        this._msvcDiagnostics = [];
    }
    ...
    createDiagnostics(build_dir) {
        const diags_by_file = new Map();
        const make_abs = (p) => util.lightNormalizePath(path.isAbsolute(p) ? p : path.join(build_dir, p));
        const severity_of = (p) => {
            switch (p) {
                case 'warning':
