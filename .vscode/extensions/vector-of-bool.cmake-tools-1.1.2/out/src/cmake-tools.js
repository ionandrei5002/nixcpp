...
class CMakeTools {
    ...
    constructor(extensionContext, workspaceContext) {
        ...
        rollbar_1.default.takePromise('Setup cache editor server', {}, ready.then(() => {
            ...
            vscode.workspace
                .registerTextDocumentContentProvider('cmake-cache', new cache_editor_1.CacheEditorContentProvider(this.extensionContext, editor_server.address().port));
        }));
    }
    ...
    async getCMakeDriverInstance() {
        return this._driverStrand.execute(async () => {
            ...
            if (cmakePath === null)
                cmakePath = '';
            const cmake = await cmake_executable_1.getCMakeExecutableInformation(cmakePath);
            if (!cmake.isPresent) {
    ...
    async _refreshCompileDatabase() {
        ...
        if (await pr_1.fs.exists(compdb_path)) {
            ...
            try {
                await pr_1.fs.copyFile(compdb_path, copy_dest);
            }
            catch (e) {
    ...
    async build(target_) {
        ...
        try {
            ...
            return await vscode.window.withProgress({
                ...
                await util_2.setContextValue(IS_BUILDING_KEY, true);
                const rc = await drv.build(target, consumer);
                await util_2.setContextValue(IS_BUILDING_KEY, false);
                if (rc === null) {
                ...
                diagnostics_1.populateCollection(this._buildDiagnostics, file_diags);
                return rc === null ? -1 : rc;
            });
        }
        finally {
